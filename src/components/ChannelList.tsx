    ...
    <div className="w-64 flex-shrink-0 flex flex-col border-r border-white/5 bg-black/20">
      ...
      {filters.length > 0 && (
        <div className="px-2 pt-1.5 pb-1 flex flex-wrap gap-1 border-b border-white/5 flex-shrink-0">
          {filters.map(f => (
            <span key={f.id} className="flex items-center gap-1 bg-blue-500/15 border border-blue-500/20 text-blue-200 rounded-full px-2 py-0.5 text-[11px]">
              {pillLabel(f)}
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {!loading && !channels.length && (
          <p className="text-center text-white/20 text-xs py-12">
            {(search || filters.length) ? 'No matches' : 'No channels found'}
          </p>
        )}
      </div>
    </div>
